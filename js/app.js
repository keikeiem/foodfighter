...
*/
	...
	function($, angular, routeConfig) {
		...
		}).service('commonVariable', ['$location', function(header, $location) {
			var commonVariable = this;
		}]);

		// 공통 컨트롤러 설정 - 모든 컨트롤러에서 공통적으로 사용하는 부분들 선언
		app.controller('CommonController', function($scope, $http, $location, commonVariable) {
			// 스타일시트 업데이트
			...
			// $http.get('data/user_menu_data.json').success(function(data) {
			// 	$scope.userMenuData = {};
			// 	userMenuData = data;
			// });
